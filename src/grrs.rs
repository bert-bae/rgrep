...
impl Grrs {
    pub fn new(args: Cli) -> Self {
        ...
        let ignored_file_patterns: Vec<String> = ignore
            .split(",")
            .map(|s| String::from(s))
            .collect();
        Grrs {
    ...
    pub fn search(&mut self) -> Result<Vec<String>, std::io::Error> {
        ...
        for path in queue {
            ...
            self.query_lines(file, matches);
        }
        return Ok(matches);
    }

    fn query_lines(&self, file: File, &mut matches: Vec<string>) {
        ...
        while let Ok(_) = reader.read_until(b'\n', &mut buf) {
            ...
            if self.args.case_sensitive {
                matching_line = line.to_lowercase().contains(&self.args.pattern.to_lowercase());
            } else {
